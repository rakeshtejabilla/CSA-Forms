...
export class AuthController {
  ...
  async getUsers(@Req() req: any) {
    ...
    if (user.role === Role.ORG_ADMIN) {
      ...
      if (!orgId) return [];
      // ORG_ADMIN only sees ENUMERATORs in their own org
      const members = await this.usersService.findAll(orgId);
      return members.filter((u: any) => u.role === Role.ENUMERATOR);
    }
  ...
  async getLogs(@Req() req: any) {
    ...
    return logs.map(log => ({
      ...
      details: log.metadata && Object.keys(log.metadata).length > 0 
        ? JSON.stringify(log.metadata) 
        : (log.entityType ? `${log.entityType} updated` : 'Action performed')
    }));
  }
}
