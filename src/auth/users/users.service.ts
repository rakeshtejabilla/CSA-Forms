...
export class UsersService {
  ...
  async getVillageAccess(userId: string) {
    const access = await this.prisma.userVillageAccess.findMany({
      where: { userId },
      select: { villageName: true },
    });
    return access.map(a => a.villageName);
  }

  async updateVillageAccess(userId: string, villages: string[]) {
    // Delete existing access
    await this.prisma.userVillageAccess.deleteMany({
      where: { userId },
    });
    ...
    if (villages && villages.length > 0) {
      ...
      await this.prisma.userVillageAccess.createMany({
        data,
      });
    }
