...
const RepeatSection = ({ field, allFields, control, register, errors }: RepeatSectionProps) => {
  ...
  return (
    <div className="bg-slate-50 border border-slate-205 rounded-2xl p-5 flex flex-col gap-4">
      <div className="flex items-center justify-between border-b border-slate-200 pb-2">
        <h3 className="text-sm font-bold text-slate-700">{field.label}</h3>
        <button
          type="button"
          onClick={() => append({})}
          className="px-3 py-1.5 bg-brand-50 hover:bg-brand-100 text-brand-600 rounded-lg text-xs font-bold transition border border-brand-200"
        >
          + Add Entry
        </button>
      </div>
      ...
      {fields.map((item, index) => (
        <div key={item.id} className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col gap-4 relative">
          {fields.length > 1 && (
            <button
              type="button"
              onClick={() => remove(index)}
              className="absolute top-3 right-3 text-xs text-rose-500 hover:text-rose-700 font-semibold"
            >
              Remove
            </button>
          )}

          <div className="text-xs font-bold text-slate-400">Entry #{index + 1}</div>
            ...
            {childFields.map((child) => (
              <div key={child.id} className="flex flex-col gap-1.5">
                <label className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide">
                  {child.label}
                  {child.required && <span className="text-rose-500">*</span>}
                </label>
                ...
                {child.type === 'number' ? (
                  <input
                    ...
                    {...register(`${field.id}.${index}.${child.id}`, { required: child.required })}
                  ...
                  <input
                    ...
                    className={inputClass}
                  />
                ) : (child.type === 'select' || child.type === 'radio' || child.type === 'dropdown') && child.options ? (
                  <select
                    {...register(`${field.id}.${index}.${child.id}`, { required: child.required })}
...
export default function DynamicFormRenderer() {
  ...
  const shouldShowField = (field: any) => {
    if (field.groupId) {
      const parent = form?.fields?.find((f: any) => f.id === field.groupId);
      if (parent && (parent.type === 'repeat' || parent.type === 'begin_repeat')) {
        return false;
      }
    }
    if (!field.conditions || !field.conditions.fieldId) return true;

    const answerValue = formValues[field.conditions.fieldId];
    if (answerValue === undefined || answerValue === null || answerValue === '') return false;

    const strAnswer = String(answerValue).trim();
    const strCondition = String(field.conditions.value || '').trim();
    const op = field.conditions.operator || 'equals';

    switch (op) {
      case 'not_equals':
        return strAnswer.toLowerCase() !== strCondition.toLowerCase();
      case 'greater_than':
        return Number(strAnswer) > Number(strCondition);
      case 'less_than':
        return Number(strAnswer) < Number(strCondition);
      ...
      case 'less_than_or_equal':
        return Number(strAnswer) <= Number(strCondition);
      case 'contains':
        return strAnswer.toLowerCase().includes(strCondition.toLowerCase());
      case 'equals':
      default:
        return strAnswer.toLowerCase() === strCondition.toLowerCase();
    }
