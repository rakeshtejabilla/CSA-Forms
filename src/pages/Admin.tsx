...
import { Shield, Users, History, UserPlus, Trash2, Loader2, Building2, Eye, EyeOff, LogIn, LogOut, UserCheck, UserX, MapPin, Database, FileText, BarChart2, RefreshCw, Upload, Download } from 'lucide-react';
...
const formatLogDetails = (log: any) => {
  ...
  switch (action) {
    ...
    case 'USER_CREATE':
      if (meta?.body) {
        return `Created user "${meta.body.name || ''}" (${meta.body.email || ''}) as ${roleLabel(meta.body.role || 'ENUMERATOR')}.`;
      }
    ...
    case 'USER_UPDATE_ROLE':
      if (meta?.body) {
        return `Changed user's role to ${roleLabel(meta.body.role || '')}.`;
      }
      return `Updated user role.`;
    case 'USER_UPDATE':
      if (meta?.body) {
        const fields = Object.keys(meta.body).filter(k => k !== 'password').join(', ');
        return `Updated user account details (modified fields: ${fields || 'none'}).`;
      }
      return `Updated user account details.`;
    case 'USER_ASSIGN_VILLAGES':
      if (meta?.body?.villages) {
        return `Assigned village access permissions: ${meta.body.villages.join(', ') || 'None'}.`;
      }
      return `Updated user village access permissions.`;
    case 'FARMER_CREATE':
      if (meta?.body) {
        return `Registered farmer "${meta.body.name || ''}" (Age: ${meta.body.age || ''}, Crop: ${meta.body.cropName || ''}, Village: ${meta.body.village || ''}).`;
      }
      return `Registered a new farmer record.`;
    case 'FARMER_UPDATE':
      if (meta?.body) {
        return `Updated farmer "${meta.body.name || ''}" details (Crop: ${meta.body.cropName || ''}, Village: ${meta.body.village || ''}).`;
      }
      return `Updated farmer details.`;
    case 'FARMER_DELETE':
      return `Deleted a farmer record from the registry.`;
    case 'SYNC_BULK':
      if (meta?.body?.submissions) {
        const count = meta.body.submissions.length;
        const names = meta.body.submissions.map((s: any) => s.data?.name || 'Unknown').filter(Boolean).join(', ');
        return `Sync'd ${count} survey submissions in bulk for: ${names || 'unspecified'}.`;
      }
      return `Sync'd survey logs.`;
    case 'SUBMISSION_CREATE':
      if (meta?.body?.data) {
        return `Submitted survey form for "${meta.body.data.name || ''}" (Crop: ${meta.body.data.cropName || ''}, Village: ${meta.body.data.village || ''}).`;
      }
      return `Submitted a new survey form.`;
    case 'SUBMISSION_UPDATE':
      return `Updated a survey submission log.`;
    case 'SUBMISSION_DELETE':
      return `Deleted a survey submission log.`;
    case 'IMPORT_DATA':
      return `Imported farmers list from spreadsheet file.`;
    case 'EXPORT_DATA':
      return `Exported survey submissions.`;
    case 'FORM_CREATE':
      if (meta?.body) {
        return `Created a new form template: "${meta.body.title || ''}".`;
      }
      return `Created a new form template.`;
    case 'FORM_EDIT':
      if (meta?.body) {
        return `Modified form template config: "${meta.body.title || ''}".`;
      }
      return `Modified form template config.`;
    case 'FORM_DELETE':
      return `Deleted a form template.`;
    case 'DASHBOARD_CREATE':
      return `Created an analytics dashboard.`;
    case 'DASHBOARD_DELETE':
      return `Deleted an analytics dashboard.`;
    case 'WIDGET_CREATE':
      return `Added a chart widget to analytics.`;
    case 'WIDGET_DELETE':
      return `Removed a chart widget from analytics.`;
    default:
      return log.details || 'Action performed.';
  }
};

const actionBadge = (action: string): { label: string; className: string; Icon: React.ElementType } => {
  if (action === 'LOGIN') return { label: 'Login', className: 'bg-emerald-100 text-emerald-700 border-emerald-200', Icon: LogIn };
  if (action === 'LOGOUT') return { label: 'Logout', className: 'bg-slate-100 text-slate-600 border-slate-200', Icon: LogOut };
  if (action === 'USER_CREATE') return { label: 'User Created', className: 'bg-blue-100 text-blue-700 border-blue-200', Icon: UserPlus };
  if (action === 'USER_DELETE') return { label: 'User Deleted', className: 'bg-rose-100 text-rose-700 border-rose-200', Icon: UserX };
  if (action === 'USER_UPDATE') return { label: 'User Updated', className: 'bg-amber-100 text-amber-700 border-amber-200', Icon: UserCheck };
  if (action === 'USER_UPDATE_ROLE') return { label: 'Role Changed', className: 'bg-purple-100 text-purple-700 border-purple-200', Icon: Shield };
  if (action === 'USER_ASSIGN_VILLAGES') return { label: 'Villages Assigned', className: 'bg-teal-100 text-teal-700 border-teal-200', Icon: MapPin };
  if (action === 'FARMER_CREATE') return { label: 'Farmer Added', className: 'bg-green-100 text-green-700 border-green-200', Icon: UserPlus };
  if (action === 'FARMER_UPDATE') return { label: 'Farmer Updated', className: 'bg-yellow-100 text-yellow-700 border-yellow-200', Icon: UserCheck };
  if (action === 'FARMER_DELETE') return { label: 'Farmer Deleted', className: 'bg-rose-100 text-rose-700 border-rose-200', Icon: UserX };
  if (action === 'SUBMISSION_CREATE') return { label: 'Survey Submitted', className: 'bg-indigo-100 text-indigo-700 border-indigo-200', Icon: FileText };
  if (action === 'SUBMISSION_UPDATE') return { label: 'Survey Updated', className: 'bg-amber-100 text-amber-700 border-amber-200', Icon: FileText };
  if (action === 'SUBMISSION_DELETE') return { label: 'Survey Deleted', className: 'bg-rose-100 text-rose-700 border-rose-200', Icon: FileText };
  if (action === 'SYNC_BULK') return { label: 'Bulk Sync', className: 'bg-cyan-100 text-cyan-700 border-cyan-200', Icon: RefreshCw };
  if (action === 'IMPORT_DATA') return { label: 'Import', className: 'bg-violet-100 text-violet-700 border-violet-200', Icon: Upload };
  if (action === 'EXPORT_DATA') return { label: 'Export', className: 'bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200', Icon: Download };
  if (action === 'FORM_CREATE') return { label: 'Form Created', className: 'bg-blue-100 text-blue-700 border-blue-200', Icon: FileText };
  if (action === 'FORM_EDIT') return { label: 'Form Edited', className: 'bg-amber-100 text-amber-700 border-amber-200', Icon: FileText };
  if (action === 'FORM_DELETE') return { label: 'Form Deleted', className: 'bg-rose-100 text-rose-700 border-rose-200', Icon: FileText };
  if (action === 'DASHBOARD_CREATE') return { label: 'Dashboard Created', className: 'bg-indigo-100 text-indigo-700 border-indigo-200', Icon: BarChart2 };
  if (action === 'DASHBOARD_DELETE') return { label: 'Dashboard Deleted', className: 'bg-rose-100 text-rose-700 border-rose-200', Icon: BarChart2 };
  if (action === 'WIDGET_CREATE') return { label: 'Widget Added', className: 'bg-teal-100 text-teal-700 border-teal-200', Icon: BarChart2 };
  if (action === 'WIDGET_DELETE') return { label: 'Widget Removed', className: 'bg-rose-100 text-rose-700 border-rose-200', Icon: BarChart2 };
  return { label: action, className: 'bg-slate-100 text-slate-600 border-slate-200', Icon: Database };
};
          ...
          <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-slate-700">Activity Log</h2>
                <span className="text-[10px] text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full font-medium">{logs.length} events</span>
              </div>
            </div>
            <div className="flex flex-col divide-y divide-slate-100 max-h-[420px] overflow-y-auto">
              {logs.length === 0 ? (
                <p className="text-xs text-slate-400 italic py-8 text-center">No activity logs available.</p>
              ) : (
                logs.map((log) => {
                  const badge = actionBadge(log.action);
                  const { date, time } = formatLogTimestamp(log.timestamp);
                  return (
                    <div key={log.id} className="flex items-start gap-3.5 px-5 py-4 hover:bg-slate-50/70 transition">
                      {/* Action Icon */}
                      <div className={`mt-0.5 p-2 rounded-xl border shrink-0 ${badge.className}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {log.userName && (
                            <span className="text-[10px] font-semibold text-slate-700">{log.userName}</span>
                          )}
                          {log.userRole && (
                            <span className={`text-[10px] px-1.5 py-0.5 rounded border font-medium ${roleBadgeClass(log.userRole)}`}>{roleLabel(log.userRole)}</span>
                          )}
                          <span className="text-[10px] text-slate-400 font-mono truncate">{log.userEmail}</span>
                        </div>
                        <p className="text-xs text-slate-600 mt-1.5 leading-relaxed">{formatLogDetails(log)}</p>
                      </div>
                      {/* Timestamp */}
                      <div className="text-right shrink-0">
                        <div className="text-[10px] text-slate-500 font-semibold whitespace-nowrap">{time}</div>
                        <div className="text-[10px] text-slate-400 whitespace-nowrap">{date}</div>
                      </div>
                    </div>
                  );
                })
