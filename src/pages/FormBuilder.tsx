  ...
  return (
    ...
    >
      ...
      {field.conditions?.fieldId && (() => {
        const triggerField = fields.find((f) => f.id === field.conditions?.fieldId);
        const symbol = getOperatorSymbol(field.conditions?.operator);
        return (
          <div className="flex items-center gap-1.5 text-[10px] text-amber-600 font-semibold mt-3 ml-14 bg-amber-50 px-2 py-1 rounded-lg border border-amber-200 w-fit">
            ...
            Shown if &quot;{triggerField?.label || triggerField?.id || 'Field'}&quot; {symbol} &quot;{field.conditions.value}&quot;
          </div>
        );
      })()}
    </div>
...
export default function FormBuilderPage() {
  ...
  useEffect(() => {
    if (selectedField) {
      ...
      const text = opts.map((opt: any) => (typeof opt === 'object' && opt !== null ? (opt.label || opt.value) : opt)).join(', ');
      const currentParsed = optionsText.split(',').map(x => x.trim()).filter(Boolean).join(', ');
      const newParsed = opts.map((opt: any) => (typeof opt === 'object' && opt !== null ? (opt.label || opt.value) : opt)).join(', ');
  ...
  }, [selectedFieldId, selectedField?.options]);
  ...
  const getFieldIcon = (type: FieldType) => {
    const map: Record<FieldType, React.ReactNode> = {
      ...
      repeat: <Layers className="h-4 w-4" />,
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="mapToFarmers"
                  checked={settings.mapToFarmers || false}
                  onChange={(e) => updateSettings({ mapToFarmers: e.target.checked })}
                  className="rounded text-brand-500 focus:ring-brand-200 border-slate-300 cursor-pointer w-4 h-4"
                />
                <label htmlFor="mapToFarmers" className="text-sm font-semibold text-slate-700 cursor-pointer select-none">
                  Map to Farmers
                </label>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2 pt-1">
                  ...
                  <p className="text-[10px] text-slate-400 leading-relaxed">
                    This field will only be <strong>visible</strong> when another field's answer matches a specific value or operator.
                  </p>
                  <div className="flex flex-col gap-3 bg-amber-50 border border-amber-100 p-3 rounded-xl">
                    {/* Step 1: Select trigger field */}
                    <div className="flex flex-col gap-1">
                      <label className="text-[10px] text-amber-700 font-bold uppercase tracking-wide">Step 1 — Which field controls this?</label>
                      <select
                        value={selectedField.conditions?.fieldId || ''}
                        onChange={(e) => {
                          updateField(selectedField.id, {
                            conditions: e.target.value
                              ? { fieldId: e.target.value, operator: 'equals', value: '' }
                              : undefined,
                          });
                        }}
                        className="bg-white border border-amber-200 rounded-lg text-xs px-2.5 py-2 w-full text-slate-700 outline-none focus:border-amber-400"
                      >
                    ...
                    {selectedField.conditions?.fieldId && (
                      <div className="flex flex-col gap-1">
                        <label className="text-[10px] text-amber-700 font-bold uppercase tracking-wide">Step 2 — Select rule type (rule)</label>
                        <select
                          value={selectedField.conditions?.operator || 'equals'}
                          onChange={(e) =>
                            updateField(selectedField.id, {
                              conditions: {
                    ...
                    {/* Step 3: What value triggers it */}
                    {selectedField.conditions?.fieldId && (() => {
                      const triggerField = fields.find(f => f.id === selectedField.conditions?.fieldId);
                      const operator = selectedField.conditions?.operator || 'equals';
                      const isEqualityOp = ['equals', 'not_equals'].includes(operator);
                      const hasOptions = triggerField && ['select', 'dropdown', 'checkbox', 'radio'].includes(triggerField.type);
                      const opts = triggerField?.options || [];

                      return (
                        <div className="flex flex-col gap-1">
                          <label className="text-[10px] text-amber-700 font-bold uppercase tracking-wide">
                            Step 3 — Expected answer value
                          </label>
                          {hasOptions && opts.length > 0 && isEqualityOp ? (
                            <select
                            ...
                            <input
                              ...
                              placeholder={
                                triggerField?.type === 'number'
                                  ...
                                  ? 'e.g. 2026-08-19'
                                  : 'Type the expected answer value...'
                              }
                              className="bg-white border border-amber-200 rounded-lg text-xs px-2.5 py-2 w-full text-slate-700 outline-none focus:border-amber-400"
                            />
                          )}
                    ...
                    {/* Preview summary */}
                    {selectedField.conditions?.fieldId && selectedField.conditions?.value && (() => {
                      const triggerField = fields.find(f => f.id === selectedField.conditions?.fieldId);
                      const getOperatorLabelText = (op?: string) => {
                        switch (op) {
                      ...
                      const opText = getOperatorLabelText(selectedField.conditions?.operator);
                      return (
                        <div className="text-[10px] text-amber-800 bg-amber-100 rounded-lg px-2.5 py-2 leading-relaxed border border-amber-200 shadow-sm">
                          ✨ <strong>"{selectedField.label || 'This field'}"</strong> will appear only when <strong>"{triggerField?.label || '...'}"</strong> {opText} <strong>"{selectedField.conditions.value}"</strong>.
                        </div>
                      );
                    })()}

                    {/* Guide Box (how to use it) */}
                    <div className="bg-white border border-amber-200 rounded-xl p-3 text-[10px] text-slate-500 leading-relaxed mt-1 flex flex-col gap-1.5 shadow-sm">
                      <span className="font-bold text-slate-700 uppercase tracking-wider">💡 How to use rules</span>
                      <ul className="list-disc list-inside flex flex-col gap-1">
                        <li><strong>Equals (=):</strong> Match exactly (e.g. show if crop = <code>Paddy</code>).</li>
                        <li><strong>Greater Than (&gt;):</strong> Show if a number is higher (e.g. show if land size &gt; <code>5</code> acres).</li>
                        <li><strong>Less Than (&lt;):</strong> Show if a number is lower (e.g. show if count &lt; <code>30</code>).</li>
                        <li><strong>Contains:</strong> Match part of the text (e.g. show if name contains <code>Reddy</code>).</li>
                      </ul>
                    </div>
                    ...
                    {selectedField.conditions?.fieldId && (
                      <button
                        type="button"
                        onClick={() => updateField(selectedField.id, { conditions: undefined })}
                        className="text-[10px] text-rose-500 hover:text-rose-700 font-semibold text-left mt-0.5 underline"
                      >
                        ✕ Remove this rule
                      </button>
                    )}
