...
export class FormsController {
  ...
  getAssignments(@Param('id') id: string, @Req() req: any) {
    if (req.user.role === 'ENUMERATOR') {
      throw new BadRequestException('Enumerators cannot view form assignments');
    }
    return this.formsService.getAssignments(id, req.user);
  }

  @Post(':id/assignments')
  assignForm(@Param('id') id: string, @Body() body: { userIds: string[] }, @Req() req: any) {
    if (req.user.role === 'ENUMERATOR') {
      throw new BadRequestException('Enumerators cannot assign forms');
    }
    return this.formsService.assignForm(id, body.userIds, req.user);
  }
  ...
  importXlsForm(@UploadedFile() file: Express.Multer.File, @Req() req: any) {
    ...
    return this.formsService.importXlsForm(file.buffer, file.originalname, req.user.sub, req.user.organizationId);
  }
}
